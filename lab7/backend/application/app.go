...
func (appl *Application) Configure(engine *gin.Engine) error {
	...
	engine.Use(observability.PrometheusMiddleware(), observability.GinTracingMiddleware())
...
func (appl *Application) Run() {
	...
	port := getHTTPPort()
	tracingShutdown, tracingError := observability.ConfigureTracing(context.Background())
	if tracingError != nil {
		fmt.Printf("Tracing disabled: %v\n", tracingError)
	}
	server := &http.Server{
		Addr:    "0.0.0.0:" + port,
		Handler: engine,
